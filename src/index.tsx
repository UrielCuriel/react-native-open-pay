...
export type OpenpayProps = {
  loading: boolean;
  merchantId: string;
  address: any;
  successToken(data: any): any;
  failToken(data: any): any;
  publicKey: string;
...
export default class Openpay extends Component<OpenpayProps, OpenpayState> {
  ...
  constructor(props: OpenpayProps | Readonly<OpenpayProps>) {
    super(props);
  ...
  createTokenWithCard = async (data: any) => {
    const method = 'POST';
  ...
  sendFunction = (method: any, resource: any, data: any) => {
    ...
    var encoded = base64.encode(username + ':');
  ...
  handleCreditCardInputs = (form: any) => {
    this.setState(() => ({ form }));
  };
    ...
    return (
      <View style={styles.container}>
        ...
        <Button
          ...
          title={this.props.buttonText ?? 'Pay Now'}
          loading={loading}
        />
        <WebView
          source={{ uri: uri }}
...
const isObject = (value: { constructor: ObjectConstructor }) => {
